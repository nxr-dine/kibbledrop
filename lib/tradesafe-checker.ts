...
export class TradesafeConfigChecker {
  static checkConfiguration() {
    ...
    if (!config.merchantId || config.merchantId === "your_tradesafe_merchant_id") {
      issues.push("TRADESAFE_MERCHANT_ID is not configured or contains placeholder value");
      status.configured = false;
    }

    if (!config.apiKey || config.apiKey === "your_tradesafe_api_key") {
      issues.push("TRADESAFE_API_KEY is not configured or contains placeholder value");
      status.configured = false;
    }

    if (!config.webhookSecret || config.webhookSecret === "your_webhook_secret") {
      issues.push("TRADESAFE_WEBHOOK_SECRET is not configured or contains placeholder value");
    }
    ...
    if (!status.configured) {
      ...
      status.recommendation = "TradeSafe is properly configured and ready to process payments.";
    }
  ...
  static async testConnection() {
    ...
    if (!status.configured) {
      console.log("❌ Cannot test connection - TradeSafe not properly configured");
      return false;
    }
    ...
    try {
      ...
      console.log("(Note: This is a connection test, no actual payment will be created)");
