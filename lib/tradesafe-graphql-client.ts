...
export class TradesafeGraphQLClient {
  ...
  constructor() {
    ...
    this.environment =
      ...
      "sandbox";
    this.authUrl =
      process.env.TRADESAFE_AUTH_URL || "https://auth.tradesafe.co.za";
    ...
    if (!this.clientId || !this.clientSecret) {
      throw new Error(
        "TradeSafe credentials not configured. Please set TRADESAFE_CLIENT_ID and TRADESAFE_CLIENT_SECRET"
      );
    }
  }
  ...
  private async getAccessToken(): Promise<string> {
    ...
    try {
      ...
      console.error(
        "❌ Failed to acquire TradeSafe OAuth access token:",
        error
      );
      throw error;
    }
  ...
  private async executeGraphQL<T = any>(
    ...
    try {
      console.log(
        `🌐 Executing TradeSafe GraphQL operation to: ${this.graphqlUrl}`
      );
  ...
  async createUserToken(userData: {
    ...
    const result = await this.executeGraphQL<{ tokenCreate: UserToken }>(
      mutation,
      variables
    );
  ...
  async getUserToken(tokenId: string): Promise<UserToken> {
    ...
    const result = await this.executeGraphQL<{ token: { data: UserToken } }>(
      query,
      { id: tokenId }
    );
  ...
  async getTransaction(transactionId: string): Promise<Transaction> {
    ...
    const result = await this.executeGraphQL<{ transaction: Transaction }>(
      query,
      { id: transactionId }
    );
