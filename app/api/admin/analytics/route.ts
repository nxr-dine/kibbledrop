...
export async function GET(request: NextRequest) {
  try {
    ...
    const customerData = {
      ...
      customersWithOrders: users.filter((user) => user.orders.length > 0)
        .length,
      customersWithSubscriptions: users.filter(
    ...
    const productData = {
      ...
      topSellingProducts: products
        .map((product) => {
          const orderSales = product.orderItems.reduce(
          ...
          const subscriptionSales = product.subscriptionItems
            ...
            .reduce(
              (sum, item) => sum + (product.price || 0) * (item.quantity || 0),
              0
            );
          return {
            id: product.id,
            ...
            orderQuantity: product.orderItems.reduce(
      ...
      subscriptionRevenue: subscriptions
        ...
        .reduce((sum, sub) => {
          return (
            ...
            sub.items.reduce(
              (itemSum, item) =>
                itemSum + (item.product.price || 0) * (item.quantity || 0),
              0
            )
          );
        }, 0),
...
function calculateRepeatCustomerRate(users: any[]): number {
  ...
  return customersWithOrders.length > 0
    ? (repeatCustomers / customersWithOrders.length) * 100
    : 0;
}
