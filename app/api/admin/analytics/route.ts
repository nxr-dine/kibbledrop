  ...
  try {
    ...
    const productData = {
      ...
      topSellingProducts: products
        .map(product => {
          const orderSales = product.orderItems.reduce((sum, item) => sum + ((item.price || 0) * (item.quantity || 0)), 0)
          const subscriptionSales = product.subscriptionItems
            .filter(item => item.subscription.status === 'active')
            .reduce((sum, item) => sum + ((product.price || 0) * (item.quantity || 0)), 0)
          return {
            id: product.id,
            ...
            orderQuantity: product.orderItems.reduce((sum, item) => sum + (item.quantity || 0), 0),
      ...
      categoryPerformance: products.reduce((acc, product) => {
        const revenue = product.orderItems.reduce((sum, item) => sum + ((item.price || 0) * (item.quantity || 0)), 0)
      ...
      petTypePerformance: products.reduce((acc, product) => {
        const revenue = product.orderItems.reduce((sum, item) => sum + ((item.price || 0) * (item.quantity || 0)), 0)
        const petType = product.petType || 'Unknown'
        acc[petType] = (acc[petType] || 0) + revenue
        return acc
      }, {} as Record<string, number>)
    }
    ...
    const revenueData = {
      ...
      subscriptionRevenue: subscriptions
        ...
        .reduce((sum, sub) => {
          return sum + sub.items.reduce((itemSum, item) => itemSum + ((item.product.price || 0) * (item.quantity || 0)), 0)
        }, 0),
