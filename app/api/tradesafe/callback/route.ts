...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!webhookSecret) {
      console.error(
        "❌ TRADESAFE_WEBHOOK_SECRET not configured in environment variables"
      );
    ...
    const eventType =
      payload.event_type || payload.data?.state || "UNKNOWN_EVENT";
    ...
    if (transactionId) {
      try {
        ...
        if (updatedTrade.count > 0) {
          console.log(
            `✅ Updated ${updatedTrade.count} trade record(s) in database`
          );
          databaseUpdate = {
            success: true,
  ...
  return NextResponse.json(
    {
      ...
      message:
        "This endpoint only accepts POST requests for TradeSafe webhooks",
    },
    { status: 405 }
  );
}
