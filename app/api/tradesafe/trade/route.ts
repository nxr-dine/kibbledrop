...
export async function POST(request: NextRequest) {
  try {
    ...
    const tokenResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/tradesafe/token`, {
      method: 'POST',
      headers: {
    ...
    console.log('🌐 Step 2: Getting GraphQL URL from environment configuration...');
    ...
    console.log('📤 Sending mutation:', JSON.stringify({ query: createTransactionMutation, variables: mutationVariables }, null, 2));
    ...
    console.log('📋 GraphQL response received:', JSON.stringify(graphqlResult, null, 2));
      ...
      try {
        ...
        const savedTrade = await prisma.trade.create({
          data: {
            ...
            buyerEmail: mutationVariables.input.parties.find(p => p.role === 'BUYER')?.contact.email || '',
            sellerEmail: mutationVariables.input.parties.find(p => p.role === 'SELLER')?.contact.email || '',
            amount: mutationVariables.input.allocations.reduce((sum, alloc) => sum + alloc.value, 0),
