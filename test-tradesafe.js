...
async function testTradeCreation() {
  ...
  if (data.success) {
    console.log(
      `🆔 Transaction ID: ${
        data.transactionData?.transactionCreate?.id || "N/A"
      }`
    );
    console.log(
      `📊 Status: ${data.transactionData?.transactionCreate?.status || "N/A"}`
    );

    if (data.databaseSave) {
      console.log(
        `💾 Database Save: ${
          data.databaseSave.success ? "✅ Success" : "❌ Failed"
        }`
      );
...
async function testWebhookEndpoint() {
  ...
  const { response, data, error } = await makeRequest(
    `${BASE_URL}/api/tradesafe/callback`,
    {
      method: "POST",
...
async function runTests() {
  ...
  console.log(
    `🔔 Webhook Security:  ${results.webhook ? "✅ PASS" : "❌ FAIL"}`
  );
  ...
  if (totalPassed === totalTests) {
    console.log(
      "🎉 All tests passed! TradeSafe integration is working correctly."
    );
  } else {
