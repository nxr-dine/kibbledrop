    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600">
          Demonstration of TradeSafe integration with OAuth authentication, trade creation, and webhook handling
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            >
              {tokenLoading ? (
                <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Getting Token...</>
              ) : (
                <>🔑 Get Access Token</>
              )}
            </Button>
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      <div className="space-y-1">
                        ...
                        <p><strong>Expires:</strong> {tokenData.expires_at}</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setTradeForm(prev => ({ ...prev, title: e.target.value }))}
                />
              </div>
              <div>
            ...
            >
              {tradeLoading ? (
                <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Creating Trade...</>
              ) : (
                <>🏗️ Create Trade</>
              )}
            </Button>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              <Badge variant="secondary">{tradeData.data?.transaction.state}</Badge>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <div className="text-sm space-y-1">
                  <p><strong>ID:</strong> {tradeData.data?.transaction.id}</p>
                  <p><strong>Reference:</strong> {tradeData.data?.transaction.reference}</p>
                  <p><strong>Currency:</strong> {tradeData.data?.transaction.currency}</p>
                  <p><strong>Created:</strong> {new Date(tradeData.data?.transaction.createdAt || '').toLocaleString()}</p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="text-sm space-y-1">
                  <p><strong>Name:</strong> {tradeData.data?.buyer.name}</p>
                  <p><strong>Email:</strong> {tradeData.data?.buyer.email}</p>
                  <p><strong>Token ID:</strong> {tradeData.data?.buyer.tokenId}</p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="text-sm space-y-1">
                  <p><strong>Name:</strong> {tradeData.data?.seller.name}</p>
                  <p><strong>Email:</strong> {tradeData.data?.seller.email}</p>
                  <p><strong>Company:</strong> {tradeData.data?.seller.organization}</p>
                  <p><strong>Token ID:</strong> {tradeData.data?.seller.tokenId}</p>
                </div>
              </div>
            </div>
            ...
            <div className="border-t pt-4">
              ...
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => window.open(tradeData.data?.paymentLink.url, '_blank')}
                >
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => copyToClipboard(tradeData.data?.paymentLink.url || '')}
                >
                  <Copy className="h-4 w-4" />
                </Button>
              </div>
              <p className="text-xs text-gray-500 mt-1">
                Expires: {new Date(tradeData.data?.paymentLink.expiresAt || '').toLocaleString()}
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <span className="bg-purple-100 text-purple-800 text-sm font-medium px-2 py-1 rounded">3</span>
            Webhook Monitoring
          </CardTitle>
          <CardDescription>
        ...
        <CardContent className="space-y-4">
          ...
          >
            {webhookLoading ? (
              <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Checking Webhooks...</>
            ) : (
          ...
          {webhookData && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p><strong>Status:</strong> <Badge variant="outline">{webhookData.status}</Badge></p>
                  <p><strong>Secret:</strong> <Badge variant="secondary">{webhookData.webhookSecret}</Badge></p>
                </div>
                <div>
                  <p><strong>Transactions:</strong> {webhookData.mockDatabase.transactionCount}</p>
                  <p className="text-sm text-gray-500">Recent webhook callbacks</p>
                </div>
              </div>
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {webhookData.mockDatabase.transactions.map((tx, index) => (
                      <div key={index} className="flex justify-between items-center text-sm border-b pb-1">
                        ...
                        <span className="text-gray-500">{new Date(tx.updatedAt).toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm text-gray-600">
          <p><strong>Environment:</strong> Sandbox mode - safe for testing</p>
          <p><strong>Webhook URL:</strong> {typeof window !== 'undefined' ? `${window.location.origin}/api/tradesafe/callback` : '/api/tradesafe/callback'}</p>
          <p><strong>Token Endpoint:</strong> /api/tradesafe/token</p>
          <p><strong>Trade Endpoint:</strong> /api/tradesafe/trade</p>
          <p className="text-xs">
            💡 To test webhooks, configure your TradeSafe application callback URL to point to the webhook endpoint above.
            For local development, use ngrok or similar tools to expose your localhost.
          </p>
        </CardContent>
      </Card>
    </div>
